...
def InitialisationSauvegarde(MotDePasse):
    ...
    NomFichier = "Saved Messages/" " Discussion from " + Jour + time.strftime("-%d-", time.localtime()) + Mois + time.strftime("-%H-%M-%S", time.localtime())
    ...
    Header = ChiffrementVignère(MotDePasse, "File decrypted !") 
    #Cette ligne nous permet de vérifier si le fichier a été déchiffré correctement
    Annonce = ChiffrementVignère(MotDePasse, str(f"Conversation started on {DateDébutConversation} at {HeureDébutConversation}."))
